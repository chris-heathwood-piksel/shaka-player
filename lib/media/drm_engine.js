...
shaka.media.DrmEngine.prototype.onSessionMessage_ = function(event) {
  ...
  var licenseTime = (new Date()).getTime();

  this.networkingEngine_.request(requestType, request)
      .then(function(response) {
        var windowsBrokenTime = 786;
        var now = (new Date()).getTime();
        var self = this;
        var timeTaken = now - licenseTime;
        var timeoutTime = windowsBrokenTime - (timeTaken % windowsBrokenTime);

        if (this.destroyed_) return Promise.reject();

        // Create a new promise and return that instead
        var licensePromise = new Promise(
